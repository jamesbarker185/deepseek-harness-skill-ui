...
// Self-contained bundle (the browser cannot require sibling files):
//   - filterSkills is inlined here; src/filter.js is the tested canonical copy.
//   - data arrives via the package-private host.call channel when available,
//     falling back to the existing connection.api.skills.list RPC; otherwise
//     the palette renders an explicit error state.
...
// Covers tickets 1-3: list + live search, full-body read pane, keyboard nav,
...
module.exports = {
  ...
  apply(ctx) {
    ...
    const callHost = (method, args) => {
      ...
      if (method === 'skills/list' && connection && connection.api && connection.api.skills && typeof connection.api.skills.list === 'function') {
        return Promise.resolve(connection.api.skills.list({}))
      }
      return Promise.reject(new Error('skills bridge unavailable (no host or connection channel)'))
    }
    ...
    const ensureLoaded = () => {
      ...
      callHost('skills/list').then((list) => {
    ...
    const openDetail = (name) => {
      ...
      callHost('skills/get', { name }).then((skill) => {
